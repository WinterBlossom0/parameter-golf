...
# HYPERPARAMETERS
# -----------------------------
# Full competition config targeting 8xH100 SXM:
# - 11 transformer blocks at width 512, 3x MLP, SmearGate, BigramHash
# - Partial RoPE (16/64), LN Scale, XSA on last 4 layers
# - EMA, int6 GPTQ-lite + zlib, sliding window eval
# - 786,432 train tokens per step, seq_len 2048, 600s wallclock cap
...
# GPTQ-lite: try multiple clip percentiles per row, pick the one minimizing reconstruction MSE.
GPTQ_CLIP_PERCENTILES = [0.999, 0.9995, 0.9999, 0.99999, 1.0]
# Int6 range: [-31, 31]
QUANT_BITS = 6
QUANT_MAX_VAL = (1 << (QUANT_BITS - 1)) - 1  # 31
# Embedding tensors use int8 for higher fidelity (they're a small fraction of params).
EMBED_NAME_PATTERNS = ("tok_emb", "lm_head")
INT8_MAX_VAL = 127
...
def _quantize_rows_gptq_lite(t32: Tensor, max_val: int) -> tuple[Tensor, Tensor]:
    # GPTQ-lite: per-row optimal clip percentile search.
    # Try multiple percentiles per row and pick the one minimizing reconstruction MSE.
    best_q = None
    best_scale = None
    best_mse = None
    ...
    return best_q.contiguous(), best_scale.to(dtype=PER_ROW_SCALE_DTYPE).contiguous()

def quantize_float_tensor(name: str, t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    # Embeddings use int8 for higher fidelity; all other matrices use int6.
    is_embed = any(pattern in name for pattern in EMBED_NAME_PATTERNS)
    max_val = INT8_MAX_VAL if is_embed else QUANT_MAX_VAL
    if t32.ndim == 2:
        return _quantize_rows_gptq_lite(t32, max_val)
...
def quantize_state_dict(state_dict: dict[str, Tensor]):
    # Export format: int6 for MLP+attention matrices, int8 for embeddings,
    # fp16 passthrough for small/control tensors, GPTQ-lite clip search.
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base, qk_gain_init, layer_idx=i, ln_scale=ln_scale,
...
def main() -> None:
    ...
    # Collect extra params from SmearGate + BigramHash
    if base_model.smear is not None:
        scalar_params.append(base_model.smear.gate)
    bigram_matrix_params: list[nn.Parameter] = []
    if base_model.bigram is not None:
        for p in base_model.bigram.parameters():
            if p.ndim >= 2 and p.numel() > 65536:
                bigram_matrix_params.append(p)
            else:
    ...
    if bigram_matrix_params:
        optimizer_bigram = torch.optim.AdamW(
            [{"params": bigram_matrix_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    # EMA weight averaging: maintain exponential moving average of all parameters.
    ...
    while True:
        ...
        # EMA update: exponential moving average of all parameters.
        if ema_state is not None:
            decay = args.ema_decay
            with torch.no_grad():
                for name, param in base_model.state_dict().items():
                    ema_state[name].mul_(decay).add_(param, alpha=1.0 - decay)
    ...
    # Swap to EMA weights for evaluation and serialization.
    if ema_state is not None:
        log0("ema:swapping to EMA weights for eval/export")
    ...
    if args.eval_stride > 0:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_sliding_s{args.eval_stride} val_loss:{sw_loss:.4f} val_bpb:{sw_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_int6_sliding_s{args.eval_stride}_exact val_loss:{sw_loss:.8f} val_bpb:{sw_bpb:.8f}")
